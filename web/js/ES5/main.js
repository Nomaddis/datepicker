    ...
    _createClass(HotelDatepicker, [{
        ...
        value: function showSelectedInfo() {
            ...
            if (this.start && this.end) {
                ...
                var _dateRangeValue = this.getDateString(new Date(this.start)) + this.separator + this.getDateString(new Date(this.end));
        ...
        value: function dayHovering(day) {
            ...
            if (!this.hasClass(day, 'datepicker__month-day--invalid')) {
                ...
                if (this.start && !this.end) {
                    var _nights = this.countDays(hoverTime, this.start) - 1;

                    if (this.hoveringTooltip) {
                        if (typeof this.hoveringTooltip === 'function') {
                            tooltip = this.hoveringTooltip(_nights, this.start, hoverTime);
                        } else if (this.hoveringTooltip === true && _nights > 0) {
                            var label = _nights === 1 ? this.lang('night') : this.lang('nights');
                            tooltip = _nights + ' ' + label;
                        }
                    }
                }
            }
...
var nights = document.getElementById('days');
...
function initbooking(input, options) {
    //let options = data;
    var datepicker = new HotelDatepicker(input, options);
    datepicker.open();
    //write days & cost qty
